...
var FileManagerThumbnailsItemList = function(_FileManagerItemListB) {
    ...
    _createClass(FileManagerThumbnailsItemList, [{
        ...
        value: function(fileItemInfo) {
            ...
            var text = item.name + "\r\n";
            if (!item.isDirectory) {
                text += _message2.default.format("dxFileManager-listThumbnailsTooltipTextSize") + ": " + (0, _uiFile_managerUtils.getDisplayFileSize)(item.size) + "\r\n"
            }
            text += _message2.default.format("dxFileManager-listThumbnailsTooltipTextDateModified") + ": " + item.dateModified;
            return text
        }
    }, {
        key: "_getUniqueId",
